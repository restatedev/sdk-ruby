...
#
# Usage:
#   ./etc/run-integration-tests.sh              # build image + run tests
...
docker run --rm \
  ...
  -v "${REPO_ROOT}/test-services/exclusions.yaml:/opt/exclusions.yaml:ro" \
  -v "${REPO_ROOT}/test-services/.env:/opt/service.env:ro" \
