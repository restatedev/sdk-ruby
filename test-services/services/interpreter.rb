...
class ServiceInterpreterHelper < Restate::Service
  ...
  handler def echoLater(ctx, req) # rubocop:disable Naming/MethodName
    ctx.sleep(req['sleep'] / 1000.0).await
    req['parameter']
  end

  handler def terminalFailure(_ctx) # rubocop:disable Naming/MethodName
    raise Restate::TerminalError, 'bye'
  end

  handler def incrementIndirectly(ctx, param) # rubocop:disable Naming/MethodName
    layer = param['layer']
    key = param['key']
    program = { 'commands' => [{ 'kind' => INCREMENT_STATE_COUNTER }] }
    raw = JSON.generate(program).b
    ctx.generic_send("ObjectInterpreterL#{layer}", 'interpret', raw, key: key)
    nil
  end

  handler def resolveAwakeable(ctx, aid) # rubocop:disable Naming/MethodName
    ctx.resolve_awakeable(aid, 'ok')
    nil
  end

  handler def rejectAwakeable(ctx, aid) # rubocop:disable Naming/MethodName
    ctx.reject_awakeable(aid, 'error')
    nil
  end

  handler def incrementViaAwakeableDance(ctx, input) # rubocop:disable Naming/MethodName
...
def interpret_program(layer, ctx, program)
  ...
  program['commands'].each_with_index do |cmd, i|
    if cmd['kind'] == AWAIT_PROMISE
      ...
      interpret_one(layer, ctx, cmd, i, coros)
    end
    await_promise.call(i)
  end
end

def interpret_one(layer, ctx, cmd, i, coros) # rubocop:disable Naming/MethodParameterName
  case cmd['kind']
  when SET_STATE then ctx.set("key-#{cmd['key']}", "value-#{cmd['key']}")
  when GET_STATE then ctx.get("key-#{cmd['key']}")
  when CLEAR_STATE then ctx.clear("key-#{cmd['key']}")
  when INCREMENT_STATE_COUNTER
    c = ctx.get('counter') || 0
    ctx.set('counter', c + 1)
  when SLEEP then ctx.sleep(cmd['duration'] / 1000.0).await
  when CALL_SERVICE
    expected = "hello-#{i}"
    coros[i] = [expected, ctx.generic_call('ServiceInterpreterHelper', 'echo',
                                           Restate::JsonSerde.serialize(expected)), true]
  when CALL_SLOW_SERVICE
    expected = "hello-#{i}"
    arg = { 'parameter' => expected, 'sleep' => cmd['sleep'] }
    coros[i] = [expected, ctx.generic_call('ServiceInterpreterHelper', 'echoLater',
                                           Restate::JsonSerde.serialize(arg)), true]
  when INCREMENT_VIA_DELAYED_CALL
    delay = cmd['duration'] / 1000.0
    arg = { 'layer' => layer, 'key' => ctx.key }
    ctx.generic_send('ServiceInterpreterHelper', 'incrementIndirectly',
                     Restate::JsonSerde.serialize(arg), delay: delay)
  when SIDE_EFFECT
    expected = "hello-#{i}"
    result = ctx.run_sync('sideEffect') { expected }
    raise Restate::TerminalError, "Expected #{expected} but got #{result}" if result != expected
  when SLOW_SIDE_EFFECT then :noop
  when RECOVER_TERMINAL_CALL then recover_terminal_call(ctx)
  when RECOVER_TERMINAL_MAYBE_UN_AWAITED then :noop # rubocop:disable Lint/DuplicateBranch
  when THROWING_SIDE_EFFECT
    ctx.run_sync('throwingSideEffect') { raise StandardError, 'Random error' if rand(2) == 1 }
  ...
  when RESOLVE_AWAKEABLE then handle_resolve_awakeable(ctx, i, coros)
  when REJECT_AWAKEABLE then handle_reject_awakeable(ctx, i, coros)
  when INCREMENT_STATE_COUNTER_VIA_AWAKEABLE
    handle_increment_via_awakeable(ctx, layer, coros)
  when CALL_NEXT_LAYER_OBJECT
    handle_call_next_layer(ctx, layer, cmd, i, coros)
  else
    raise Restate::TerminalError, "Unknown command type: #{cmd['kind']}"
  end
end

def recover_terminal_call(ctx)
  ctx.generic_call('ServiceInterpreterHelper', 'terminalFailure',
...
def handle_resolve_awakeable(ctx, idx, coros)
  aid, promise = ctx.awakeable
  coros[idx] = ['ok', promise, false]
  ctx.generic_send('ServiceInterpreterHelper', 'resolveAwakeable',
                   Restate::JsonSerde.serialize(aid))
end

def handle_reject_awakeable(ctx, idx, coros)
  aid, promise = ctx.awakeable
  coros[idx] = ['rejected', promise, false]
  ctx.generic_send('ServiceInterpreterHelper', 'rejectAwakeable',
                   Restate::JsonSerde.serialize(aid))
end

def handle_increment_via_awakeable(ctx, layer, _coros)
  tx_aid, tx_promise = ctx.awakeable
  arg = { 'interpreter' => { 'layer' => layer, 'key' => ctx.key }, 'txPromiseId' => tx_aid }
  ctx.generic_send('ServiceInterpreterHelper', 'incrementViaAwakeableDance',
                   Restate::JsonSerde.serialize(arg))
  their_aid = tx_promise.await
  ctx.resolve_awakeable(their_aid, 'ok')
end

def handle_call_next_layer(ctx, layer, cmd, idx, coros)
  next_layer = "ObjectInterpreterL#{layer + 1}"
  key = cmd['key'].to_s
  raw = JSON.generate(cmd['program']).b
  coros[idx] = [''.b, ctx.generic_call(next_layer, 'interpret', raw, key: key), false]
end
...
class ObjectInterpreterL0 < Restate::VirtualObject # rubocop:disable Style/OneClassPerFile
  ...
  handler def interpret(ctx, program)
    interpret_program(self.class._layer, ctx, program)
    nil
  end

  shared def counter(ctx)
    ctx.get('counter') || 0
  end
end
